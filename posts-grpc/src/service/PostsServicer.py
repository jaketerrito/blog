...
from database.model.post import Post as PostDocument
...
class PostsServicer(posts_pb2_grpc.PostsServicer):
    ...
    def convert_document_to_proto(self, post_document: PostDocument) -> Post:
        return Post(
            id=post_document.id,
            title=post_document.title,
            content=post_document.content,
            created_at=Timestamp().FromDatetime(post_document.created_at),
            updated_at=Timestamp().FromDatetime(post_document.updated_at)
        )

    def GetPost(self, request: GetPostRequest, context: grpc.ServicerContext) -> GetPostResponse:
        try:
            post_document = self.posts_repository.get_post(request.id)
        except PostNotFoundError as e:
            context.abort(grpc.StatusCode.NOT_FOUND, str(e))
        return GetPostResponse(
            post=self.convert_document_to_proto(post_document)
        )
    ...
    def GetPosts(self, request: GetPostsRequest, context: grpc.ServicerContext) -> GetPostsResponse:
        ...
        return GetPostsResponse(posts=[self.convert_document_to_proto(post) for post in posts])
    ...
    def UpdatePost(self, request: UpdatePostRequest, context: grpc.ServicerContext) -> UpdatePostResponse:
        try:
            post = self.posts_repository.update_post(request.post.id, request.post.title, request.post.content)
        except PostNotFoundError as e:
            context.abort(grpc.StatusCode.NOT_FOUND, str(e))
        return UpdatePostResponse(post=self.convert_document_to_proto(post))
